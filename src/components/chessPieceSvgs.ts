// Cburnett chess piece SVG set (public domain).
// Source: https://github.com/lichess-org/lila/tree/master/public/piece/cburnett
// Designed by Colin M.L. Burnett (Wikimedia Commons). The de-facto standard
// chess piece set used by lichess.org, Wikipedia and many open chess projects.
//
// ViewBox is 0 0 45 45 for all pieces. White pieces use fill="#fff" with a black
// stroke; black pieces use fill="#000" with a black stroke and light interior
// accents (#ececec). Each SVG below is a self-contained piece glyph.
...
// When rendering a black piece on a dark square in dark mode, the default
// black stroke disappears. We swap the outer stroke for a lighter color so
// the piece silhouette remains readable. Cburnett's wQ/wR/wB pieces fill
// black-stroked white shapes, so we leave those alone — the black outline
// against any board color is already readable.
export function tunePieceSvg(
  svg: string,
  pieceColor: 'w' | 'b',
  themeMode: 'light' | 'dark',
): string {
  if (pieceColor === 'b' && themeMode === 'dark') {
    // Replace the black outline on black pieces with a near-white outline.
    // Keep the #ececec internal accents as-is.
    return svg.replace(/stroke="#000"/g, 'stroke="#f5ecd5"');
  }
  if (pieceColor === 'w' && themeMode === 'light') {
    // White stroke on light squares can sometimes look thin — leave default
    // black outline which provides good contrast on cream squares.
    return svg;
  }
  return svg;
}
